extends Node
## analog400mw 2.0.1 loads through zonemods now. This file is only here so an
...
func _init() -> void:
	if ProjectSettings.has_setting("autoload/ZoneMods"):
		return
	var msg := "analog400mw 2.0.1 is not loaded: override.cfg in the game folder is from an older version. Replace it with the one from the analog400mw zip."
